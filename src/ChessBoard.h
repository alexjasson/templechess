...
/* Squares store only piece types (Empty, Pawn, King, Knight, Bishop, Rook, Queen).
   Colors are represented in colorBB bitboards. */
 ...
 */
/* A square stores only a piece type (Empty, Pawn, King, Knight, Bishop, Rook, Queen). */
...
/* Returns the bitboard of the given piece type for the side to move */
static inline BitBoard ChessBoardPieces(ChessBoard *cb, Type t)
{
  return cb->pieceTypeBB[t] & cb->colorBB[ChessBoardColor(cb)];
}
